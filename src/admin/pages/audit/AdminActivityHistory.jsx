...
export default function AdminActivityHistory() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setIsLoading(true)
        setError('')
        const params = { type: 'admin', page, limit, sortBy: 'timestamp', sortOrder: 'desc' }
        ...
        setError(e.message || 'Failed to load admin activity')
      } finally {
        setIsLoading(false)
      }
    }
  ...
  const getActionBadgeColor = (action) => {
    switch (action?.toLowerCase()) {
      case 'login': return 'bg-green-50 text-green-700 border border-green-200';
      case 'logout': return 'bg-gray-50 text-gray-700 border border-gray-200';
      case 'create': return 'bg-blue-50 text-blue-700 border border-blue-200';
      ...
      case 'view': return 'bg-purple-50 text-purple-700 border border-purple-200';
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-semibold text-gray-900 mb-1">Admin Activity History</h1>
              <p className="text-sm text-gray-600">
                Monitor all administrative actions and system activities • {total} total activities
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3">
                ...
                <tr>
                  <td className="px-6 py-8 text-center text-sm text-gray-500" colSpan={5}>
                    {search || actionFilter !== 'all' || dateFilter !== 'all'
                      ...
                      : 'No admin activities found. Activities will appear here as admins perform actions.'
                    }
                  </td>
                </tr>
              ) : (
                logs.map((log) => (
                  <tr key={log._id || log.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-900">
                      {log.resource || log.metadata?.resource || 'N/A'}
                    </td>
