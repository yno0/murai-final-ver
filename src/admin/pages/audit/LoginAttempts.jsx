  ...
  useEffect(() => {
    const load = async () => {
      try {
        setIsLoading(true)
        setError('')
        // We'll fetch type=admin and search includes 'login'/'logout' on the server side via action if provided
        const params = { type: 'admin', page, limit, sortBy: 'timestamp', sortOrder: 'desc' }
        if (search) params.search = search
        const res = await adminApiService.getLogs(params)
        const items = (res?.data?.logs || res?.logs || []).filter(l => /admin_login|admin_logout/i.test(l.action || ''))
